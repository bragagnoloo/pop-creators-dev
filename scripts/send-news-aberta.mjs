/**
 * Disparo: "Nova campanha POPline News está aberta"
 *
 * Público:
 ...
 *
 * Uso:
 *   node scripts/send-news-aberta.mjs           # dry-run (mostra contagem, não envia)
 *   node scripts/send-news-aberta.mjs --send    # envia de verdade
 *
 * Requer no .env.local:
...
const SUBJECT = 'Nova campanha POPline News está aberta';
...
function buildText(nome) {
  ...
  return `Olá ${primeiroNome}, uma nova campanha do POPline News acabou de abrir!

As candidaturas já estão liberadas na plataforma — entre agora e garanta a sua participação.

Acesse agora: ${CTA_URL}
          ...
          <tr>
            <td style="padding:36px 40px 28px;">
              ...
              <p style="margin:0 0 20px;font-size:22px;font-weight:700;color:#111118;line-height:1.3;">
                Olá ${primeiroNome}! Uma nova campanha POPline News está aberta.
              </p>

              <p style="margin:0 0 16px;font-size:15px;color:#444455;line-height:1.7;">
                As candidaturas para a nova campanha do POPline News já estão liberadas dentro da plataforma. Entre agora e garanta a sua participação.
              </p>
...
async function main() {
  ...
  if (!SHOULD_SEND) {
    ...
    console.log('   node scripts/send-news-aberta.mjs --send\n');
    return;
  }
