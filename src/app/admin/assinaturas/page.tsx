...
export default function AdminAssinaturasPage() {
  ...
  const { data: stats } = useSWR('/api/admin/subscriptions/stats', fetcher);
  ...
  const chartData = (stats?.dailyNew ?? []).map((d: { date: string; count: number }, i: number) => ({
    date: d.date,
    novos: d.count,
    ativos: stats?.dailyActive?.[i]?.count ?? 0,
  }));
  ...
  function exportCSV() {
    ...
    const headers = ['Nome', 'Email', 'Plano', 'Método', 'Cadastro', 'Assinatura', 'Vence', 'Status', 'Origem', 'Renovações'];
    const lines = rows.map((r: Record<string, unknown>) => [
      ...
      r.status, r.utmSource ?? '', r.renewalCount,
    ].join(','));
  ...
  return (
    <div className="space-y-8 py-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card>
          <p className="text-xs text-text-secondary mb-1">Novos (30d)</p>
          <p className="text-3xl font-bold">{stats?.newLast30 ?? '—'}</p>
        </Card>
        <Card>
          <p className="text-xs text-text-secondary mb-1">Churn (30d)</p>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <Card className="md:col-span-2">
          <h3 className="text-sm font-semibold mb-4">Assinantes por dia — últimos 30 dias</h3>
          <LineChart
            data={chartData}
            lines={[
              ...
              return (
                <div key={plan}>
                  ...
                  <div className="w-full bg-border rounded-full h-1.5">
                    <div
                      className="bg-popline-pink h-1.5 rounded-full transition-all"
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border text-left">
                ...
                <th className="py-3 px-2 font-medium text-text-secondary">Origem</th>
                <th className="py-3 px-2 font-medium text-text-secondary">Renovações</th>
                <th className="py-3 px-2 font-medium text-text-secondary">Auto-renova</th>
              </tr>
            </thead>
            <tbody className="[&>tr]:border-b [&>tr]:border-border/40">
              {(list?.data ?? []).map((r: Record<string, unknown>) => (
                <tr key={r.userId as string} className="hover:bg-surface/50">
                  ...
                  <td className="py-3 px-2">
                    <span className="font-medium">{PLANS[r.plan as PlanId]?.name ?? r.plan as string}</span>
                  </td>
                  <td className="py-3 px-2 text-text-secondary">
                    {methodLabel(r.paymentMethod as string)}
                  </td>
                  <td className="py-3 px-2 text-text-secondary">
                    {r.firstSubscribedAt
                      ? new Date(r.firstSubscribedAt as string).toLocaleDateString('pt-BR')
                      : '—'}
                  </td>
                  <td className="py-3 px-2 text-text-secondary">
                    {r.expiresAt
                  ...
                  <td className="py-3 px-2 text-text-secondary text-xs">
                    {(r.utmSource as string) ?? (r.utmCampaign as string) ?? '—'}
                  </td>
                  <td className="py-3 px-2 text-center text-text-secondary">
                    {r.renewalCount as number}
                  </td>
              ...
              {!list && (
                <tr>
                  <td colSpan={9} className="py-8 text-center text-text-secondary text-sm">
                    Carregando...
                  </td>
                </tr>
              )}
        ...
        {list && list.total > 50 && (
          <div className="flex items-center justify-between mt-4 text-sm">
            <span className="text-text-secondary">
              {list.total} assinantes no total
            </span>
            <div className="flex gap-2">
              <Button variant="secondary" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1}>
                Anterior
              </Button>
              <span className="flex items-center px-3 text-text-secondary">
                Página {page} de {Math.ceil(list.total / 50)}
              </span>
              <Button variant="secondary" onClick={() => setPage((p) => p + 1)} disabled={page >= Math.ceil(list.total / 50)}>
                Próxima
              </Button>
            </div>
          </div>
      ...
      {expiringSoon.length > 0 && (
        <Card>
          <h3 className="text-sm font-semibold mb-4">
            Próximos vencimentos — 7 dias ({expiringSoon.length})
          </h3>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
