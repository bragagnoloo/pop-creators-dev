...
function LessonModal({
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (videoUrl) {
      ...
      const isSupabase = videoUrl.includes('/storage/v1/object/public/');
      if (!isYoutube && !isSupabase) {
        setUrlError('Cole uma URL válida do YouTube ou do Supabase Storage.');
        return;
      }
    }
    setSaving(true);
    const data = { title, description, youtubeUrl: videoUrl };
    if (editing) {
      ...
      <form onSubmit={handleSave} className="space-y-4">
        ...
        <div>
          <Input
            ...
            placeholder="YouTube ou URL do Supabase Storage"
          />
          {urlError && <p className="text-xs text-red-400 mt-1">{urlError}</p>}
          <p className="text-xs text-text-secondary mt-1">
            Para hospedar no Supabase: Storage → videos → upload → copiar URL pública.
          </p>
        </div>
        <div className="flex gap-3">
          <Button type="button" variant="secondary" className="flex-1" onClick={onClose}>Cancelar</Button>
