-- =============================================================================
-- 0034 — B2B: creators contratados no acordo
-- =============================================================================
--
-- Acrescenta o número de creators que foi CONTRATADO com a marca, para poder
-- comparar com quantos de fato ficaram aptos.
--
--   * campaign_finance.contracted_creators — preenchido à mão (vem do contrato,
--     o banco não tem como saber).
--   * creators_gap na view — contratados menos aptos. Negativo significa que
--     entregou mais gente do que o combinado; positivo, que faltou.
--
-- O "total a pagar a creators" continua sendo aptos × cachê: contratado é o
-- combinado comercial, aptos é quem realmente gera custo.
--
-- A view precisa ser recriada para expor as colunas novas.
-- =============================================================================

drop view if exists b2b_finance_overview;

alter table campaign_finance
  add column if not exists contracted_creators integer
    check (contracted_creators is null or contracted_creators >= 0);
