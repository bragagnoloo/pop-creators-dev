-- =============================================================================
-- 0035 — Campanha Radar (POPline Creators Radar)
-- =============================================================================
--
-- Quarta categoria de campanha, 100% aditiva. Mesma mecânica do Review (0030):
--
--   * Nova coluna is_radar default false → toda campanha atual continua igual.
--   * Radar é PÚBLICA (is_invite = false): herda a policy pública de leitura e a
--     policy RESTRICTIVE "hide invite from non-participants" (que para
--     is_invite = false avalia sempre true), e a RPC apply_with_term como está —
--     ela só barra is_invite e status <> 'open'. add_campaign_guest e
--     remove_campaign_guest já rejeitam campanha não-convite.
--     Portanto NÃO tocamos em RLS nem em nenhuma RPC.
--   * A distinção é visual/organizacional (sub-aba própria dentro de Campanhas,
--     moldura laranja neon), feita na aplicação a partir da flag is_radar.
--
-- Esta migration é inócua para o app hoje em produção: ele não conhece is_radar
-- e continua funcionando idêntico. Pode (e deve) ser aplicada ANTES do deploy do
-- código — o acoplamento perigoso é o inverso, porque o C_SELECT do
-- services/campaigns.ts passa a pedir a coluna.
-- =============================================================================
...
-- Index parcial: acelera qualquer filtro por campanhas radar (poucas linhas).
create index if not exists campaigns_is_radar_idx on campaigns(is_radar) where is_radar;

-- -----------------------------------------------------------------------------
-- 2. Exclusão mútua entre os TRÊS tipos: no máximo uma flag ligada.
--
--    O constraint da 0030 era pairwise — not (is_invite and is_review) — e não
--    escala para uma terceira flag. Reescrito como cardinalidade.
--
--    As três colunas são NOT NULL, então o cast boolean->int é seguro
--    (true::int = 1, false::int = 0) e não há aritmética com NULL.
--
--    NOT VALID + validate: as linhas existentes têm no máximo uma flag (o
--    constraint antigo garantia isso, e is_radar acabou de nascer false), então
--    a validação não pode falhar em dado existente.
-- -----------------------------------------------------------------------------

alter table campaigns drop constraint if exists campaigns_type_exclusive;

alter table campaigns
  add constraint campaigns_type_exclusive
  check ((is_invite::int + is_review::int + is_radar::int) <= 1) not valid;

alter table campaigns validate constraint campaigns_type_exclusive;

-- -----------------------------------------------------------------------------
-- 3. b2b_finance_overview — recriada APENAS para acrescentar o ramo 'radar' ao
--    case de campaign_type. O resto do corpo é idêntico ao da 0034.
--
--    Sem essa recriação, campanha Radar apareceria como 'standard' no /admin/b2b,
--    silenciosamente.
--
--    A view não tem objeto dependente (b2b_finance_platform lê withdrawals e
--    balance_credits direto), então o drop não precisa de cascade. Mas o drop
--    APAGA os grants — daí o revoke/grant no fim, como na 0032/0033/0034.
--
--    A ordem do case (review → radar → invite → standard) tem que bater com o
--    DETECTION_ORDER de src/lib/campaign-categories.ts.
-- -----------------------------------------------------------------------------
...
select
  ...
  case
    when c.contracted_creators is null then null
    else (c.contracted_creators - c.eligible_participants)
  end as creators_gap,
  c.credited_total,
  c.processing_total,
  c.wallet_total,
...
-- Garante que o PostgREST enxergue campaigns.is_radar e a view recriada.
